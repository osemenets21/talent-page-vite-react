...
// Try MySQL first, then SQLite, then fallback to file-based DB
try {
    require_once 'EventsMysqlDB.php';
    $db = new EventsMysqlDB($dbConfig['host'], $dbConfig['dbname'], $dbConfig['username'], $dbConfig['password']);
} catch (Exception $mysqlError) {
    try {
        require_once 'EventsDB.php';
        $db = new EventsDB();
    } catch (Exception $sqliteError) {
        // Both SQL options failed, use file-based database
...
try {
    ...
    switch ($method) {
        ...
        case 'POST':
            ...
            $timeFields = ['doors_open_time', 'show_start_time', 'show_end_time'];
            foreach ($timeFields as $field) {
                if (isset($input[$field]) && !empty($input[$field])) {
                    // Accept both HH:MM and HH:MM:SS formats
                    if (!preg_match('/^\d{2}:\d{2}(:\d{2})?$/', $input[$field])) {
                        http_response_code(400);
                        echo json_encode(['success' => false, 'message' => "Invalid time format for $field. Use HH:MM or HH:MM:SS"]);
                        exit;
                    }
                    // Convert HH:MM to HH:MM:SS for MySQL
                    if (strlen($input[$field]) === 5) {
                        $input[$field] .= ':00';
                    }
                }
            }
